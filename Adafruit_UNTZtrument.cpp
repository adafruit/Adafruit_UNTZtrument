/*!
 * @file Adafruit_UNTZtrument.cpp
 *
 * @mainpage Arduino library for UNTZtrument - a Trellis button controller.
 *
 ...
 * href="https://github.com/adafruit/https://github.com/adafruit/Adafruit_Trellis_Library">
 * Adafruit_Trellis</a> being present on your system. Please make sure you have
...
Adafruit_UNTZtrument::Adafruit_UNTZtrument(
    ...
    : Adafruit_TrellisSet(matrix0, matrix1, matrix2, matrix3, matrix4, matrix5,
                          matrix6, matrix7) {
  size = matrix4 ? 128 : 64;
}
...
static const uint8_t PROGMEM
    i2xy64[] =
        { // Remap 8x8 TrellisSet button index to column/row
            0x00, 0x10, 0x20, 0x30, 0x01, 0x11, 0x21, 0x31, 0x02, 0x12, 0x22,
            0x32, 0x03, 0x13, 0x23, 0x33, 0x40, 0x50, 0x60, 0x70, 0x41, 0x51,
            0x61, 0x71, 0x42, 0x52, 0x62, 0x72, 0x43, 0x53, 0x63, 0x73, 0x04,
            0x14, 0x24, 0x34, 0x05, 0x15, 0x25, 0x35, 0x06, 0x16, 0x26, 0x36,
            0x07, 0x17, 0x27, 0x37, 0x44, 0x54, 0x64, 0x74, 0x45, 0x55, 0x65,
            0x75, 0x46, 0x56, 0x66, 0x76, 0x47, 0x57, 0x67, 0x77},
    i2xy128[] =
        { // Remap 16x8 TrellisSet button index to column/row
            },
    xy2i64[8][8] =
        { // Remap [row][col] to Trellis button/LED index
    ...
    xy2i128[8][16] = {
        ...
        {68, 69, 70, 71, 84, 85, 86, 87, 100, 101, 102, 103, 116, 117, 118,
         119},
        {72, 73, 74, 75, 88, 89, 90, 91, 104, 105, 106, 107, 120, 121, 122,
         123},
        {76, 77, 78, 79, 92, 93, 94, 95, 108, 109, 110, 111, 124, 125, 126,
         127}};
...
#define ENCODER_WRAP 1 ///< Flag if encoder wrap-around is enabled
...
void enc::setBounds(enc_t lo, enc_t hi, boolean wrap) {
  ...
  if (wrap) {
    flags |= ENCODER_WRAP;
    if (value < lo)
      value = hi - ((lo - (value + 1)) % (hi - lo + 1));
    else if (value > hi)
      value = lo + ((value - (hi + 1)) % (hi - lo + 1));
  } else {
    flags &= ~ENCODER_WRAP;
    if (value < lo)
      value = lo;
...
void enc::setValue(enc_t v) {
  if (v < min) {
    value = (flags & ENCODER_WRAP) ? (max - ((min - (v + 1)) % (max - min + 1)))
                                   : min;
  } else if (v > max) {
    value = (flags & ENCODER_WRAP) ? (min + ((v - (max + 1)) % (max - min + 1)))
                                   : max;
  } else {
    value = v;
  }
...
enc_t enc::getValue(void) { return value; }
...
void enc::poll(void) {
  for (enc *e = list; e; e = e->next) {
    ...
    if (*e->pinRegA & e->pinMaskA)
      e->state |= 2; // New state
    if (*e->pinRegB & e->pinMaskB)
      e->state |= 1;
    switch (e->state & B1111) {
    case B0000:
    case B0101:
    case B1010:
    case B1111: // No change
      e->x2 = 0;
      break;
    case B0001:
    case B0111:
    ...
    case B1110: // Left
      ...
      e->x2 = -2;
      if (e->value < e->min) {
        e->value = (e->flags & ENCODER_WRAP) ? e->max : e->min;
      }
      break;
    case B0010:
    case B0100:
    ...
    case B1101: // Right
      ...
      e->x2 = 2;
      if (e->value > e->max) {
        e->value = (e->flags & ENCODER_WRAP) ? e->min : e->max;
      }
      break;
    case B0011:
    case B0110:
    case B1001:
    case B1100:          // Ambiguous
      e->value += e->x2; // Use 2X prior motion
      if (e->value < e->min) {
        e->value =
            (e->flags & ENCODER_WRAP)
                ? e->max - ((e->min - (e->value + 1)) % (e->max - e->min + 1))
                : e->min;
      } else if (e->value > e->max) {
        e->value =
            (e->flags & ENCODER_WRAP)
                ? e->min + ((e->value - (e->max + 1)) % (e->max - e->min + 1))
                : e->max;
      }
      break;
    }
